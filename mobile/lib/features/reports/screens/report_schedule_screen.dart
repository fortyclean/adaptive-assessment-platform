...
extension _FileFormatExt on _FileFormat {
  ...
  IconData get icon =>
      this == _FileFormat.pdf ? Icons.picture_as_pdf_rounded : Icons.table_chart_rounded;
}
...
class _ReportScheduleScreenState
    extends ConsumerState<ReportScheduleScreen> {
  // Form state
  bool _isActive = true;
    ...
    return Container(
      ...
      child: Column(
        ...
        children: [
          ...
          // Classroom chips (Screen 33 variant)
          _FormLabel(label: 'اختيار الفصول'),
          const SizedBox(height: 8),
          _ClassroomChips(
          ...
          // Recipients
          _FormLabel(label: 'البريد الإلكتروني للمستلمين'),
          const SizedBox(height: 8),
          _RecipientsInput(
    ...
    return Container(
      ...
      child: DropdownButtonHideUnderline(
        child: DropdownButton<_ReportType>(
          value: value,
          isExpanded: true,
          icon: const Icon(Icons.expand_more_rounded, color: Color(0xFF505F76)),
          style: AppTextStyles.bodyMedium.copyWith(color: const Color(0xFF1A1B22)),
          onChanged: (v) {
            if (v != null) onChanged(v);
          },
          items: _ReportType.values.map((rt) {
        ...
        GestureDetector(
          ...
          child: Container(
            ...
            child: Row(
              mainAxisSize: MainAxisSize.min,
              children: [
                const Icon(Icons.add_rounded, size: 18, color: Color(0xFF505F76)),
                const SizedBox(width: 4),
                Text(
                  'إضافة فصل',
                  style: AppTextStyles.labelLarge.copyWith(
