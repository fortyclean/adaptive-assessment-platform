  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const skillHeatmap = Array.from(skillTotals.entries()).map(([mainSkill, stats]) => ({
        mainSkill,
        averagePercentage:
          stats.total > 0 ? Math.round((stats.correct / stats.total) * 100 * 100) / 100 : 0,
      }));
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.setHeader(
        'Content-Disposition',
        `attachment; filename="assessment-${req.params.id}-results.csv"`,
      );
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const studentMap = new Map(
        (
          classroom.studentIds as unknown as {
    ...
    try {
      ...
      const comparison = classroomStats.map((c) => {
        ...
        const completionRate =
          enrolledTotal > 0 ? Math.round((c.enrolledCount / enrolledTotal) * 100 * 100) / 100 : 0;

        // Determine top skill: flatten all skill breakdowns and find highest avg
        const skillTotals = new Map<string, { total: number; correct: number }>();
        for (const breakdownArray of c.allSkillBreakdowns as {
          mainSkill: string;
...
router.get(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res.status(200).json({
        exportNote:
          'PDF rendering is handled client-side. This structured JSON contains all data required to generate the report.',
