  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if ((error as { code?: number }).code === 11000) {
        res.status(409).json({
          error: 'A question with this text already exists in the same subject, grade, and unit',
        });
        return;
      }
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      if (!subject || !gradeLevel || !unit) {
        res
          .status(400)
          .json({ error: 'subject, gradeLevel, and unit are required query parameters' });
        return;
      }
      ...
      const qualityReport = {
        ...
        isAdaptiveReady:
          easyCnt >= MIN_PER_DIFFICULTY &&
          mediumCnt >= MIN_PER_DIFFICULTY &&
          hardCnt >= MIN_PER_DIFFICULTY,
        warnings: [] as string[],
      };

      if (easyCnt < MIN_PER_DIFFICULTY)
        qualityReport.warnings.push(
          `Insufficient easy questions: ${easyCnt}/${MIN_PER_DIFFICULTY}`,
        );
      if (mediumCnt < MIN_PER_DIFFICULTY)
        qualityReport.warnings.push(
          `Insufficient medium questions: ${mediumCnt}/${MIN_PER_DIFFICULTY}`,
        );
      if (hardCnt < MIN_PER_DIFFICULTY)
        qualityReport.warnings.push(
          `Insufficient hard questions: ${hardCnt}/${MIN_PER_DIFFICULTY}`,
        );
...
router.post(
  ...
  async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      res
        .status(500)
        .json({ error: (error as Error).message || 'An internal server error occurred' });
    }
  },
);
